...
def create(db: Session, order: OrderCreate):
    if order.customer_id:
        ...
        if not (order.guest_name and order.guest_email):
            raise HTTPException(status_code=400, detail="Guest info required for guest checkout")
    ...
    for item in menu_items:
        for mii in item.menu_item_ingredients:
            ...
            required_ingredients[ingr.id] += mii.quantity
    ...
    if order.promotion_id:
        promo_obj = db.query(promotion.Promotion).filter(
            promotion.Promotion.id == order.promotion_id,
            promotion.Promotion.expiration_date >= datetime.utcnow()
        ).first()
        if not promo_obj:
            raise HTTPException(
                status_code=400,
                detail="Invalid or expired promotion"
            )
    ...
    for payment_data in order.payments:
        db_payment = model_payment.Payment(
...
def read_by_customer(db: Session, customer_id: int):
    return db.query(model.Order).filter(model.Order.customer_id == customer_id).all()

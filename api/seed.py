...
def seed_if_needed():
    ...
    if db.query(Customer).count() == 0:
        ...
        bacon_wrap = MenuItem(name="Bacon Wrap", category="Main", price=8.50, calories=450)
        veggie_plate = MenuItem(name="Veggie Plate", category="Appetizer", price=5.50, calories=200)
        ...
        order1 = Order(date=datetime(2025, 8, 1, 12, 0, 0), status="pending", total=17.50, order_type="takeout", tracking_number=123456, customer_id=alice.id)
        order2 = Order(date=datetime(2025, 8, 2, 13, 0, 0), status="completed", total=10.00, order_type="dine-in", tracking_number=654321, customer_id=bob.id)
        order3 = Order(date=datetime(2025, 8, 3, 14, 0, 0), status="completed", total=9.00, order_type="delivery", tracking_number=789012, customer_id=carol.id)
        order4 = Order(date=datetime(2025, 8, 4, 15, 0, 0), status="pending", total=6.00, order_type="takeout", tracking_number=210987, customer_id=dave.id)
        order5 = Order(date=datetime(2025, 8, 5, 16, 0, 0), status="completed", total=8.50, order_type="dine-in", tracking_number=345678, customer_id=eve.id)
        order6 = Order(date=datetime(2025, 8, 6, 17, 0, 0), status="completed", total=12.00, order_type="delivery", tracking_number=456789, customer_id=frank.id)

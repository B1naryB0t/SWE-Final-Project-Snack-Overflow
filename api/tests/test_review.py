def test_update_review(client, test_data):
	payload = {
		...
		"menu_item_id": test_data["menu_item_ids"][0],
		"rating": 5,
		"comment": "Updated review!"
	}
	before = client.get(f"/review/{test_data['review_ids'][0]}")
	r = client.put(f"/review/{test_data['review_ids'][0]}", json=payload)
	assert r.status_code == 200
	assert r.json()["comment"] != before.json()["comment"]


def test_read_review_by_menu_item(client, test_data):
	r = client.get(f"/review/menu/{test_data['menu_item_ids'][0]}")
	assert r.status_code == 200


def test_delete_review(client, test_data):
	r = client.delete(f"/review/{test_data['review_ids'][0]}")
...
def test_post_review(client, test_data):
	...
	get_r = client.get(f"/review/menu/{test_data['menu_item_ids'][0]}")

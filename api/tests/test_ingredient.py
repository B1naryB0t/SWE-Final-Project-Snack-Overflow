def test_delete_ingredient(client, test_data):
	r = client.delete(f"/ingredient/{test_data['ingredient_ids'][0]}")
	assert r.status_code in (200, 204)
	assert client.get(f"/ingredient/{test_data['ingredient_ids'][0]}").status_code in (404, 422)
...
def test_track_ingredient_inventory(client, test_data):
	...
	ingredient = next((ing for ing in ingredients if ing["id"] == test_data["ingredient_ids"][0]), None)
...
def test_ingredient_stock_validation(client, test_data):
	ingredient_id = test_data["ingredient_ids"][0]
